...
def create_fuec_model(session: snowpark.Session): 

    @sproc(name="create_fuec_model", is_permanent=True, stage_location="@model_stage", replace=True, packages=["snowflake-snowpark-python"], session=session)
    def create_fuec_model(session: snowpark.Session, database: str, schema: str) -> str:
        ...
        df_fuec_model_format_table.show()
        ...
        WITH Model_input_temp2 AS (
            SELECT 
                ...
                case 
                    when observed_holiday_ind = 'Y' then 9 
                    else dayofweek(ADMIT_DTTM) 
                end as model_feature3, 
                admit_type as model_feature4, 
                case 
                    when location = 'PACU Mandell' then location 
                    else 'Occupied Bed' 
                end as model_feature5, 
                service_code as model_feature6, 
        ...
        fractional_final as( 
            SELECT 
                s.model_feature1, s.scaffold_beddeddays, m.patients, s.model_feature2, s.model_feature3, s.model_feature4, s.model_feature5, s.model_feature6, s.model_feature7, 
        ...
        emergencies_patients as (
            select avg(patients) patients, service_name, date_part(month,day) month,
                dayofweek, admit_type, location, service_code, site_desc
        ...
        emergencies_patients_beddeddays_temp as (
            select day, dayofweek, day_of_week, date_part(month,d.day) month, 
                ep.service_name, ep.service_code, ep.site_desc, ep.patients, scaffold_beddeddays,
            ...
            where day >=  CURRENT_DATE
                ...
                and ep.patients > 0
            order by day, date_part(month,d.day), dayofweek(day)
        )
        ...
        select day, DAYOFWEEK, day_of_week, month, service_name, service_code, patients, scaffold_beddeddays as BEDDEDDAYS, STAY_RATE, admit_type, location, site_desc,
            ...
            sum(stay_rate) over (partition by day, service_name, admit_type, 
        ...
        success = "FUEC Model Creation Complete!"
        
        return success

    # Test w/ .sql worksheet in Snowflake UI
    # CALL HAKKODA_DB.SANDBOX.CREATE_FUEC_MODEL('HAKKODA_DB', 'SANDBOX')
